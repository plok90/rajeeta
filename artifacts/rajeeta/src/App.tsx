...
function ToothDiagram({ selected, onSelect, treated }: { selected: number; onSelect: (tooth: number) => void; treated: Record<number, ToothTreatment> }) {
  const toothButton = (tooth: number, angle: number, direction: 'upper' | 'lower') => <button type="button" key={tooth} className={`tooth arch-tooth ${direction} ${selected === tooth ? 'selected' : ''} ${treated[tooth]?.type ? 'has-treatment' : ''}`} style={{ transform: `rotate(${angle}deg) translateY(${direction === 'upper' ? -132 : 132}px) rotate(${-angle}deg)` }} onClick={() => onSelect(tooth)} aria-label={`السن ${tooth}`} data-testid={`button-new-tooth-${tooth}`}>{tooth}</button>;
