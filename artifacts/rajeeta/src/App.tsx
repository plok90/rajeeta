...
function ToothDiagram({ selected, onSelect, treated }: { selected: number; onSelect: (tooth: number) => void; treated: Record<number, ToothTreatment> }) {
  const toothButton = (tooth: number) => <button type="button" key={tooth} className={`tooth ${selected === tooth ? 'selected' : ''} ${treated[tooth]?.type ? 'has-treatment' : ''}`} onClick={() => onSelect(tooth)} aria-label={`السن ${tooth}`} data-testid={`button-new-tooth-${tooth}`}>{tooth}</button>;
  return <div className="new-tooth-diagram"><div className="diagram-labels"><span>الفك العلوي</span><small>يمين المريض</small></div><div className="mouth new-mouth"><div className="mouth-row">{TOOTH_NUMBERS.slice(0, 16).map(toothButton)}</div><div className="mouth-center"><span>الفك العلوي</span><i>اختر السن للتوثيق</i></div><div className="mouth-row">{TOOTH_NUMBERS.slice(16).map(toothButton)}</div><div className="diagram-labels lower"><span>الفك السفلي</span></div></div><div className="chart-legend-row"><span><i className="legend-dot clear" /> سليم</span><span><i className="legend-dot" /> السن المحدد</span><span><i className="legend-dot alert" /> تم اختيار علاج</span></div></div>;
}
